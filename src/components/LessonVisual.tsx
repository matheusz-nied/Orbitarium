...
export function LessonVisual({ visual }: LessonVisualProps) {
  ...
  if (visual === "cmb-observer-past") {
    return <CmbObserverPastVisual />;
  }

  if (visual === "cmb-plasma") {
    return <CmbPlasmaVisual />;
  }

  if (visual === "cmb-cooling") {
    return <CmbCoolingVisual />;
  }

  if (visual === "cmb-recombination") {
    return <CmbRecombinationVisual />;
  }

  if (visual === "cmb-redshift") {
    return <CmbRedshiftVisual />;
  }

  if (visual === "cmb-evidence") {
    return <CmbEvidenceVisual />;
  }

  if (visual === "cmb-temperature-map") {
    return <CmbMapVisual />;
  }

  if (visual === "cmb-structures") {
    return <CmbStructuresVisual />;
  }

  if (visual === "cmb-timeline") {
    return <CmbTimelineVisual />;
  }

  if (visual === "cmb-missions") {
    return <CmbMissionsVisual />;
  }

  if (visual === "cmb-measurements") {
    return <CmbMeasurementsVisual />;
  }

  if (visual === "cmb-summary") {
    return <CmbSummaryVisual />;
  }
  ...
  return (
    <figure className="rounded-[2rem] border border-sky-400/25 bg-[#0b1026] p-4 shadow-xl shadow-sky-950/30">
      <svg viewBox="0 0 760 320" role="img" aria-label="Comprimento de onda sendo esticado pela expansão" className="w-full">
        ...
        <path d="M90 110c30-55 60 55 90 0s60-55 90 0 60 55 90 0 60-55 90 0 60 55 90 0 60-55 90 0" fill="none" stroke="#facc15" strokeWidth="6" strokeLinecap="round" />
